...
def keyword_sentence(file_name, number):
    ...
    sentence_df.to_csv("huggingface model for comparison/frank_2022_sentence_0" + str(number+1) + ".csv", index=False)
...
def keyword_sentence_double(file_name, number):
    ...
    sentences = re.findall(r'\b.+?[.!?]+(?:\s|\n\n|$)', text)
    sentence_df = pd.DataFrame({"sentence": sentences})
    sentence_df.to_csv("huggingface model for comparison/frank_2022_sentence_" + str(number) + ".csv", index=False)
...
for i in range(9):
    frank_2022 = keyword_sentence("Year 2022/0" + str(i+1) + "_frank.txt", i)
    res = classifer(frank_2022)
    df = pd.DataFrame({"sentence": frank_2022, "sentiment": res})
    df.to_csv("huggingface model for comparison/frank_2022_SA_0" + str(i+1) + ".csv", index=False)

for i in range(10, 13):
    frank_2022 = keyword_sentence_double("Year 2022/" + str(i) + "_frank.txt", i)
    res = classifer(frank_2022)
    df = pd.DataFrame({"sentence": frank_2022, "sentiment": res})
    df.to_csv("huggingface model for comparison/frank_2022_SA_" + str(i) + ".csv", index=False)
